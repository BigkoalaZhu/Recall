...
double IDSC_descriptor::dist(const Point &pt1,const Point &pt2) {
	double xDist=pt1.x-pt2.x;
	double yDist=pt1.y-pt2.y;
	return(sqrt(xDist*xDist+yDist*yDist));	
}

double IDSC_descriptor::angle(const Point &p1,const Point &p2) {
	double vX=p2.x-p1.x;
	double vY=p2.y-p1.y;
...
#define NUM_PTS 20
void IDSC_descriptor::getFullShapeContext(double *sc,const std::vector<Point> &contour) 
{	
/*	int num=0;
	...
	bool orientation=getContourOrientation(contour);
	for(int n=0;n<contour.size();n++) {
		getShapeContext(sc,contour,n,orientation);
		sc+=64;
	}
}

bool IDSC_descriptor::intersect(const Point &p1a,const Point &p1b,const Point &p2a,const Point &p2b) {
...
int IDSC_descriptor::moveLeft(int n, int size) {
	if(--n<0) {
		n=size-1;
	}
	return(n);
}
int IDSC_descriptor::moveRight(int n, int size) {
	if(++n==size) {
		n=0;
	}
	return(n);
}
void IDSC_descriptor::getContext(double *sc) {
	for(int k=0;k<64;k++) {
		sc[k]=0;	
	}
...
void IDSC_descriptor::addShapeDist(double dist, double angle) {
	while(angle<0) {
		angle+=M_PI*2;
	}
	while(angle > 2*M_PI) {
...
bool IDSC_descriptor::getContourOrientation(const std::vector<Point> &contour) {
...
void IDSC_descriptor::getShapeContext(double *sc,const std::vector<Point> &contour, int ptNum, bool orientation) {
	maxDist=0;
...
void IDSC_descriptor::normalizeShapeContext(double *sc, int pt_num)
{
...
cv::Mat IDSC_descriptor::graphSc(double *sc, int width, int height)
{
	double max=-1;
	double min=1e6;
	for(int n=0;n<64;n++) {
...
Eigen::MatrixXf IDSC_descriptor::CostMat(double *sc1, double *sc2, int width)
{
	Eigen::MatrixXf mat = Eigen::MatrixXf::Zero(width,width);
	...
	for (int i = 0; i < width; i++)
	{
		for (int j = i; j < width; j++)
		{
			...
			for (int k = 0; k < 64; k++)
			{
				d += pow(sc1[i*64 + k] - sc2[j*64 + k],2)/(sc1[i*64 + k] + sc2[j*64 + k]);
			}
			d = d/2;
			mat(i,j) = d;
			mat(j,i) = d;
...
vector<Point> IDSC_descriptor::simpleContour( const Mat& currentQuery, int n=300 )
{
...
void IDSC_descriptor::getShapeContext(double *sc, const Mat& currentQuery, int pt_num=300)
{
	vector<Point> contour = simpleContour(currentQuery, pt_num);
	getFullShapeContext(sc, contour);
	normalizeShapeContext(sc, pt_num);
}
